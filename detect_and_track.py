...
def main():
    ...
    parser.add_argument('--headless', help='run program without display', action='store_true')
    ...
    while cap.isOpened():
        ...
        interpreter.invoke()
        boxes, scores, classes, count = get_packed_output(interpreter, score_threshold=args.threshold, top_k=args.top_k)
        converted_boxes = converted_boxes(pil_im, boxes)

        #enkodiraj bbox u feature za deepsort
        features = encode_features(pil_im, converted_boxes)
